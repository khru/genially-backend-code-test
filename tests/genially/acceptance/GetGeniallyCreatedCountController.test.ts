...
describe("Metrics genially counter", () => {
  ...
  it("should return 0 when there is no genially created", async () => {
    const res = await agent.get("/metrics/genially/created").expect(200).expect("Content-Type", /json/);
  ...
  it("should count 2 genially even if one is deleted", async () => {
    ...
    await agent.post("/genially").send({ id: firstId, name: "first genially name" }).expect(201);

    await agent.post("/genially").send({ id: "second-genially-id", name: "second genially name" }).expect(201);

    await agent.delete(`/genially/${firstId}`).expect(204);
