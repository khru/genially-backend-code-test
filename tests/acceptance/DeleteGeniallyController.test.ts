...
describe("Delete Genially Controller", () => {
  ...
  it("DELETE /genially/:id returns 404 when id does not exist", async () => {
    const response = await agent.delete("/genially/non-existent-id").expect("Content-Type", /json/).expect(404);
  ...
  it("DELETE /genially/:id returns 412 when already deleted", async () => {
    ...
    const res = await agent.delete("/genially/delete-already-deleted-id").expect("Content-Type", /json/).expect(412);
