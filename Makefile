...
MIG_ENV := --env-file=.env.local

# ------------------------------------------------------------
# Utility functions (macros) to be embedded in recipes
# ------------------------------------------------------------

# nvm executor (DIR, CMD) – $(2) must be a simple command string
define NVM_EXEC
	@if [ -f "$(1)/.nvmrc" ]; then
...
.PHONY: show-env
show-env: ## Print env info, .env presence, and Node versions from .nvmrc (root and migrations)
	@set -e
...
local-migrate-status: ## Show migration status
	@set -e
	if [ -d "$(MIG_DIR)" ]; then
	  echo "🗄️  Migration status"
	  $(call NVM_EXEC,$(MIG_DIR),npm $(MIG_ENV) run status)
	else
	  echo "🗄️  No mongo-migrations folder (skipped)"
	fi

.PHONY: local-migrate-up
local-migrate-up: ## Apply pending migrations
	@set -e
	if [ -d "$(MIG_DIR)" ]; then
	  echo "🗄️  Applying migrations"
	  $(call NVM_EXEC,$(MIG_DIR),npm $(MIG_ENV) run migrate)
	else
	  echo "🗄️  No mongo-migrations folder (skipped)"
	fi

.PHONY: local-migrate-down
local-migrate-down: ## Roll back last migration
	@set -e
	if [ -d "$(MIG_DIR)" ]; then
	  echo "🗄️  Rolling back last migration"
	  $(call NVM_EXEC,$(MIG_DIR),npm $(MIG_ENV) run down)
	else
	  echo "🗄️  No mongo-migrations folder (skipped)"
	fi

.PHONY: local-migrate-down-all
local-migrate-down-all: ## Roll back all migrations
	@set -e
	if [ -d "$(MIG_DIR)" ]; then
	  echo "🗄️  Rolling back all migrations"
	  $(call NVM_EXEC,$(MIG_DIR),npm $(MIG_ENV) run down:all)
	else
	  echo "🗄️  No mongo-migrations folder (skipped)"
	fi
...
onboarding: ## First-time setup: env, hooks, deps, start Docker, wait API, migrate if present
	...
	if [ -d "$(MIG_DIR)" ]; then
	  $(MAKE) local-migrate-install
	else
	  echo "🗄️  migrations not present (skipped install)"
	fi
