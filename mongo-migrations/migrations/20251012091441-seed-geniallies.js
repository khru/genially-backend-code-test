module.exports = {
  ...
  async up(db) {
    ...
    const dateFromEpochMilliseconds = (ms) => new Date(Number(ms));
    ...
    const docs = geniallies.map((genially) => ({
      ...
      createdAt: dateFromEpochMilliseconds(genially.createdAt),
      ...(genially.modifiedAt ? { modifiedAt: dateFromEpochMilliseconds(genially.modifiedAt) } : {}),
      ...(genially.deletedAt ? { deletedAt: dateFromEpochMilliseconds(genially.deletedAt) } : {}),
    }));
